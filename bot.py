        ...
        try:
            ...
            try:
                ...
                async for m in app.search_messages(src_id, filter=enums.MessagesFilter.DOCUMENT, limit=0):
                    ...
                    processed_stage2 += 1
                    try:
                        # FIX: Broadened MIME check to include mkv/other video types often labeled application/octet-stream
        ...
        try:
            ...
            try:
                ...
                async for m in app.search_messages(src_id, filter=enums.MessagesFilter.DOCUMENT, limit=0):
                    ...
                    processed_stage2 += 1
                    try:
                        # FIX: Broadened MIME check to include mkv/other video types labeled application/octet-stream
                        if not (m.document and (m.document.mime_type.startswith("video/") or (m.document.file_name and m.document.file_name.lower().endswith(('.mkv', '.mp4', '.avi', '.wmv', '.mov', '.ts', '.m4v', '.webm'))))): continue
