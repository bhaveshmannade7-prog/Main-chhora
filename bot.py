...
--- **📺 WEB SERIES COMMANDS** ---
* `/index_webseries <chat_id>` - Source ko scan karke *sorted* `webseries_database.json` banata hai.
* `/index_target_webseries <chat_id>` - Target ko scan karke `target_webseries_index.json` banata hai.
* `/clear_webseries_index` - Source webseries index (`.json`) delete karta hai.
* `/clear_target_webseries_index` - Target webseries index (`.json`) delete karta hai.
* `/forward_webseries <target_chat_id> [limit]` - Sorted webseries ko forward karta hai.

--- **✒️ CAPTION EDITING COMMANDS** ---
...
* `/stop_all` - Bot ke sabhi current tasks (indexing, forwarding, editing) ko rokta hai.
...
async def batch_edit_captions(message, edit_mode):
    ...
    try:
        ...
        try:
            for i, batch in enumerate(batches):
                ...
                try:
                    ...
                    messages_dict = {msg.id: msg for msg in messages_in_batch if msg.caption is not None} # Check for None
                except Exception as e:
                ...
                for msg_id in batch:
                    ...
                    try:
                        ...
                        try:
                            ...
                            failed_count += 1
                            if first_error_message is None:
                                first_error_message = str(e_retry)
                    except Exception as e:
                        error_text = str(e)
                        if "MESSAGE_NOT_MODIFIED" in error_text or "CAPTION_EMPTY" in error_text:
                            ...
                            failed_count += 1
                            if first_error_message is None:
                                first_error_message = error_text
        ...
        if failed_count > 0 and first_error_message:
            ...
            if "MESSAGE_ID_INVALID" in first_error_message:
                final_message_text += f"\n**Tippani:** 'MESSAGE_ID_INVALID' ka matlab hai ki bot channel (ID: `{chat_id}`) me admin nahi hai, ya uske paas 'Edit messages' ki permission nahi hai."
...
while True:
    try:
        ...
        app.run() # Yeh line code ko yahaan rok kar rakhegi jab tak bot chalta hai
        print("Bot ruk gaya hai. 10 second me restart kar raha hoon...")
    
    except RPCError as rpc_e:
        print(f"❌ [CRITICAL RPCError] Pyrogram client crash hua: {rpc_e}")
        print("Restarting in 60 seconds...")
        time.sleep(60) # Lambi neend RPC errors ke liye
    ...
    time.sleep(10) # 10 second ka delay restart se pehle
