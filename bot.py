...
def get_media_details(m):
    # OPTIMIZATION: Check for ANY media that has file_name/size/unique_id.
    # This is more robust against undocumented file types or non-standard video documents.
    media = m.video or m.document
    if not media:
        return None, None, None
    
    # Quick check if it's explicitly audio or photo which we usually skip
    if m.audio or m.photo:
         return None, None, None
    ...
    if m.document and m.document.mime_type and not m.document.mime_type.startswith("video/") and not m.document.mime_type == "application/x-bittorrent":
         # Skip documents that are likely not video (e.g., zip, apk, text)
         pass 
