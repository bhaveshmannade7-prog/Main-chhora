...
def get_media_details(m):
    # Optimisation: Check for both video and document media types
    media = m.video or (m.document if m.document and m.document.mime_type and m.document.mime_type.startswith("video/") else None)
    if not media:
...
async def index_channel_cmd(_, message):
    ...
    async def runner():
        ...
        try:
            ...
            # --- Indexing is now Newest to Oldest (Default) for compatibility ---
            ...
            try:
                ...
                async for m in app.get_chat_history(src_id, limit=0, offset_id=0):
                    ...
                    # FIX: Status update frequency reduced for faster indexing (2000 messages)
...
async def index_full_cmd(_, message):
    ...
    async def runner():
        ...
        try:
            ...
            # --- Indexing is now Newest to Oldest (Default) ---
