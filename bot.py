...
from pyrogram.errors import (
    ...
    RPCError, CaptionEmpty, MessageNotModified
)
...
# --- NAYA REGEX: Caption cleaning ke liye ---
# Yeh links (http, https, t.me) aur usernames (@username) ko dhoondhta hai
...
def load_locked_content():
    """Loads locked_content from file on startup"""
...
--- **✒️ CAPTION EDITING COMMANDS (Naya)** ---
* `/set_locked_content <text>` - Text set karein jo delete nahi hoga / add hoga (e.g., `Join: @MyChannel`).
* `/clear_locked_content` - Locked text ko hatata hai.
* `/index_for_editing <chat_id>` - **(Pehla Step)** Channel ko scan karke `editing_index.json` banata hai.
* `/clean_captions [limit]` - **(JSON Index Chahiye)** Captions se sabhi links/usernames hatata hai (locked ko chhodkar).
* `/add_locked_content [limit]` - **(JSON Index Chahiye)** Captions ke niche locked text add karta hai.
* `/replace_all_content [limit]` - **(JSON Index Chahiye)** Pehle clean karta hai, fir locked text add karta hai.
...
async def index_for_editing_cmd(_, message):
    ...
    async def runner():
        ...
        status = await message.reply(f"⏳ **Editing Indexer** shuru ho raha hai: `{src_name}`...\n(Sirf caption wale messages index honge)\n(Stage 1: Videos)")
        ...
        try:
            ...
            # Save reverse-chronological order (newest first)
            # Taki edit karte time purane se naya edit ho (user preference)
            message_ids_with_captions.reverse()
            ...
            await status.edit(f"🎉 Editing Indexing Complete!\nChannel: `{src_name}`\nFound: **{found_count}** messages with captions.\n\nDatabase ko `{EDITING_INDEX_DB_FILE}` me save kar diya hai.")

        except Exception as e:
            await status.edit(f"❌ Editing Indexing Error: `{e}`")

    app.loop.create_task(runner())


async def batch_edit_captions(message, edit_mode):
    """Generic function to handle all caption editing modes"""
    
    if not os.path.exists(EDITING_INDEX_DB_FILE):
        await message.reply(f"❌ `editing_index.json` file nahi mili. Pehle `/index_for_editing <chat_id>` chalao.")
        return
    ...
    if total_messages == 0:
        await message.reply("ℹ️ Index me koi message ID nahi mili process karne ke liye.")
        return
    ...
    status = await message.reply(
        ...
        f"Operation 100 ke batch me shuru ho raha hai..."
    )
        ...
        for i, batch in enumerate(batches):
            ...
            for msg_id in batch:
                ...
                if msg_id not in messages_dict:
                    # Message might have been deleted or has no caption
                    skipped_count += 1
                    continue
                    
                original_caption = messages_dict[msg_id].caption
                current_caption = original_caption # Start with original
                ...
                try:
                    ...
                    if edit_mode == "clean" or edit_mode == "replace":
                        # Step 1: Clean the caption
                        if locked_content:
                            # Protect locked content
                            placeholder = "___LOCKED_CONTENT_PLACEHOLDER___"
                            temp_caption = current_caption.replace(locked_content, placeholder)
                            ...
                            # No lock, clean everything
                            final_caption = LINK_USERNAME_REGEX.sub("", current_caption).strip()
                    else:
                        final_caption = current_caption # For "add" mode
                    
                    if edit_mode == "add" or edit_mode == "replace":
                        # Step 2: Add locked content if not present
                        if locked_content and locked_content not in final_caption:
                            final_caption += f"\n\n{locked_content}" # User ne "niche" kaha tha
                            final_caption = final_caption.strip()
                            needs_edit = True # Mark for edit
                    
                    # --- Final Check ---
                    if final_caption is None:
                        final_caption = current_caption # Should not happen, but safety
                        
                    if final_caption != original_caption:
                        needs_edit = True
                        
                    if not final_caption: # Agar caption poora empty ho gaya
                        needs_edit = True # Needs edit to empty
                    
                    # --- Execution ---
                    if needs_edit:
                        await app.edit_message_caption(chat_id, msg_id, caption=final_caption)
                        edited_count += 1
                    else:
                        skipped_count += 1
                        
                except (CaptionEmpty, MessageNotModified):
                    skipped_count += 1 # No changes needed or already empty
                except FloodWait as e:
                    await status.edit(f"⏳ FloodWait... Batch {i+1} jaari hai.\nSleeping for {e.value}s.")
                    await asyncio.sleep(e.value)
                    try: # Retry
                        await app.edit_message_caption(chat_id, msg_id, caption=final_caption)
                        edited_count += 1
            ...
            await asyncio.sleep(2) # 2 sec break between batches

    except ChatAdminRequired:
        await status.edit(f"❌ **Error: Main Admin nahi hoon!**\nMujhe channel `{chat_name}` (ID: `{chat_id}`) mein 'Edit Messages' permission ke saath Admin banao.")
        return
    except Exception as e:
        await status.edit(f"❌ Ek bada error aaya: {e}")
        return

    await status.edit(
        f"🎉 **Caption Editing Complete!** (Mode: `{edit_mode.upper()}`)\n"
        f"Channel: `{chat_name}`\n\n"
        f"Total Processed: {processed_count}/{total_messages}\n"
...
async def status_cmd(_, message):
    ...
    await message.reply(
        ...
        f"--- **✒️ Editing (Naya)** ---\n"
        f"Locked Content: `{locked_content[:50]}...`\n"
...
load_webseries_duplicate_dbs()
load_movie_index_db()
load_locked_content() # Naya loader
