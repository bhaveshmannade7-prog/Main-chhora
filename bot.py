...
from pyrogram.errors import (
    ...
    RPCError
) # <-- CaptionEmpty aur MessageNotModified yahaan se hata diye (Error Fix)
...
limit_messages = None
forwarded_count = 0
GLOBAL_TASK_RUNNING = False # Naya Universal Stop Flag (is_forwarding ko replace kiya)
mode_copy = True
PER_MSG_DELAY = 0.5 
...
* `/stop_all` - **(Naya)** Bot ke sabhi current tasks (indexing, forwarding, editing) ko rokta hai.
    ...
    async def runner():
        ...
        try:
            ...
            try:
                ...
                if not GLOBAL_TASK_RUNNING: return # Stop before stage 2
                ...
                if not GLOBAL_TASK_RUNNING: return # Stop before save
            ...
            GLOBAL_TASK_RUNNING = False # Clear flag no matter what
    ...
    async def runner():
        ...
        try:
            ...
            try:
                ...
                if GLOBAL_TASK_RUNNING: # Agar stop nahi hua toh final message
...
async def forward_webseries_cmd(_, message):
    ...
    GLOBAL_TASK_RUNNING = True # Ab task start karo

    async def runner():
        global forwarded_count, GLOBAL_TASK_RUNNING
        status = initial_reply # Status message ko runner me le lo
        try:
            try:
...
async def start_forward(_, message):
    ...
    async def runner():
        ...
        try:
            ...
            GLOBAL_TASK_RUNNING = False # Task khatam, flag clear karo

    GLOBAL_TASK_RUNNING = True # Set flag before starting task
    app.loop.create_task(runner())
# -----------------------------
...
async def batch_edit_captions(message, edit_mode):
    ...
    try:
            ...
            for i, batch in enumerate(batches):
                ...
                for msg_id in batch:
                    ...
                    try:
                        ...
                        if not final_caption and original_caption: # Agar caption poora empty ho gaya
                            needs_edit = True
                        
                        if needs_edit:
                            await app.edit_message_caption(chat_id, msg_id, caption=final_caption if final_caption else ".") # Use "." for truly empty caption
                            if not final_caption:
                                await app.edit_message_caption(chat_id, msg_id, caption="") # Then set to empty
                            edited_count += 1
                        else:
                            skipped_count += 1
                ...
                if not GLOBAL_TASK_RUNNING: break # Outer loop break
        ...
        final_message = f"🎉 **Caption Editing Complete!** (Mode: `{edit_mode.upper()}`)\n"
        if not GLOBAL_TASK_RUNNING:
            final_message = f"🛑 **Caption Editing Stopped!** (Mode: `{edit_mode.upper()}`)\n"
        ...
        GLOBAL_TASK_RUNNING = False # Task khatam
