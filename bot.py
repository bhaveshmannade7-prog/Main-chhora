...
from pyrogram.errors import (
    ...
    RPCError, UsernameInvalid, ChannelPrivate # UsernameInvalid import kiya gaya
)
...
# --- resolve_chat_id फ़ंक्शन में सुधार ---
async def resolve_chat_id(client: Client, ref: str | int):
    ...
    except UsernameInvalid: # विशिष्ट रूप से इस एरर को पकड़ा गया
...
async def forward_bad_quality_cmd(_, message):
    ...
    async def runner():
        ...
        status = None
        try:
            # --- FIX: resolve_chat_id error handling ---
            try:
...
async def sync_chats(_, message):
    ...
    async def runner():
        ...
        try:
            # anext() का उपयोग करने के लिए Pyrogram v2+ में 'from pyrogram.types.user_and_chats.messages import anext' 
            # की आवश्यकता नहीं है क्योंकि यह बिल्ट-इन Python 3.10+ फ़ंक्शन है।
            # यदि आप पुराने Python version पर हैं, तो आपको 'from pyrogram.utils import anext' को 
            # कोड की शुरुआत में जोड़ना पड़ सकता है, लेकिन हम मान रहे हैं कि आप Python 3.10+ का उपयोग कर रहे हैं।
            dialog_iterator = app.get_dialogs()
            
            while True:
                try:
                    # Pyrogram v2 के async iterator का सुरक्षित उपयोग
                    ...
                    # जब लूप सफलतापूर्वक समाप्त हो जाता है
                    break 
                    
                except (PeerIdInvalid, RPCError, ChannelPrivate) as e:
                    # RPCError (जैसे CHANNEL_PRIVATE) को पकड़कर skip करें
