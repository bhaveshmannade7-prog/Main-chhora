...
async def index_channel_cmd(_, message):
    ...
    async def runner():
        ...
        try:
            ...
            # --- FIX: app.get_chat_history with reverse=True for oldest-first scan ---
            status = await message.reply(f"⏳ Movie Indexing shuru ho raha hai: `{src_name}`...\n(यह बड़े channels को पूरी तरह से **Oldest to Newest** scan करेगा)\n(Web series skip kar dega)")

            processed_count = 0
            found_count = 0
            
            try:
                # Messages को iterate करें (Oldest to Newest)
                async for m in app.get_chat_history(src_id, limit=0, offset_id=0, reverse=True):
...
async def index_target_cmd(_, message):
    ...
    async def runner():
        ...
        try:
            ...
            try:
                # Messages को iterate करें (Oldest to Newest)
                async for m in app.get_chat_history(tgt_id, limit=0, offset_id=0, reverse=True):
                    if not GLOBAL_TASK_RUNNING:
                        await status.edit("🛑 Task stopped by user.")
                        break
    ...
    async def runner():
        ...
        try:
            ...
            try:
                ...
                # FIX: Removed redundant reverse() call since get_chat_history(reverse=True) is used
...
async def clean_dupes_cmd(_, message):
    ...
    async def runner():
        ...
        try:
            ...
            try:
                # Messages को iterate करें (Oldest to Newest)
                async for m in app.get_chat_history(chat_id, limit=0, offset_id=0, reverse=True):
                    if not GLOBAL_TASK_RUNNING:
                        await status.edit("🛑 Task stopped by user.")
                        break
    ...
    async def runner():
        ...
        try:
            ...
            try:
                ...
                # Index is already in Oldest to Newest order (due to reverse=True)
