...
async def index_channel_cmd(_, message):
    ...
    async def runner():
        ...
        try:
            ...
            # --- FIX: app.get_chat_history is used for reliable full channel scan ---
            status = await message.reply(f"⏳ Movie Indexing shuru ho raha hai: `{src_name}`...\n(यह बड़े channels को पूरी तरह से scan करेगा)\n(Web series skip kar dega)")

            processed_count = 0
            found_count = 0
            
            try:
                # Messages को iterate करें
    ...
    async def runner():
        ...
        try:
            ...
            try:
                ...
                # BUG FIX: Removed reverse(). Index is now oldest-to-newest if using offset_id=0, or newest-to-oldest by default Pyrogram setting.
