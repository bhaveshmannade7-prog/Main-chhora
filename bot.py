...
def get_media_details(m):
    # OPTIMIZED: Check for ANY media that carries file metadata, regardless of exact MIME type/object, 
    # but prioritize video/document over simple audio/photo if possible for cleaner indexing.
    
    # 1. Prioritize Video object
    media = m.video
    if media:
        return media.file_name, media.file_size, media.file_unique_id

    # 2. Check Document object (and apply basic filtering for non-video documents)
    media = m.document
    if media:
        if media.mime_type and not media.mime_type.startswith("video/") and not media.mime_type in ["application/x-bittorrent", "application/zip", "application/pdf"]:
             # Skip common non-video document types directly
             pass
        else:
             return media.file_name, media.file_size, media.file_unique_id
             
    # 3. Skip other media types like Photo, Audio, Sticker, Voice, etc.
        ...
        try:
            ...
            try:
                ...
                async for m in app.get_chat_history(src_id, limit=0, offset_id=0):
                    ...
                    # Media check (Optimized get_media_details used - Should now catch all videos/docs/etc.)
                    file_name, file_size, unique_id = get_media_details(m)
                    if not unique_id: 
                        continue 
        ...
        try:
            ...
            try:
                ...
                await status.edit("❌ **Error: Main Admin nahi hoon!**\nMujhe channel/group mein 'Delete Messages' permission ke saath Admin banao.")
                 ...
                 await status.edit("❌ Scan Failed: यह channel/group content forwarding को प्रतिबंधित करता है। आप messages को retrieve नहीं कर सकते।")
            except Exception as e:
