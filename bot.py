...
async def clear_webseries_index_cmd(_, message):
    ...
    try:
        if os.path.exists(WEBSERIES_INDEX_DB_FILE):
            ...
            await message.reply(f"ℹ️ Source webseries index pehle se ही khaali hai.")
    except Exception as e:
        ...
        try:
            async for dialog in app.get_dialogs():
                try:
                    ...
                    # RPCError को पकड़कर skip करें ताकि लूप चलता रहे
                    print(f"❌ [SYNC ERR] Skipping dialog due to error: {e}")
                    skipped += 1
                    continue
            await status.edit(f"✅ User Account Cache synced! Found {count} chats. (Skipped: {skipped} errors)")
        except Exception as e:
            # अगर get_dialogs() ही क्रैश हो जाए (जो uncommon है)
