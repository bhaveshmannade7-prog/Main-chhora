...
async def sync_chats(_, message):
    ...
    async def runner():
        ...
        # Pyrogram v2 में get_dialogs() का उपयोग करते समय
        # RPCError को लूप के अंदर पकड़ने से क्रैश को रोका जा सकता है।
        # खास तौर पर CHANNEL_PRIVATE जैसे errors को handle करने के लिए।
        try:
            # हम get_dialogs() को सीधे iterate कर रहे हैं
            async for dialog in app.get_dialogs():
                try:
                    # बस dialog को एक्सेस करने की कोशिश करें
                    _ = dialog.chat.id
                    count += 1
                except (PeerIdInvalid, RPCError) as e:
                    # PeerIdInvalid या RPCError (जैसे CHANNEL_PRIVATE) को पकड़कर skip करें
            ...
            # यह बाहरी ब्लॉक तब चलेगा जब get_dialogs() खुद ही क्रैश हो जाए (जो uncommon है)
