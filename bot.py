...
from pyrogram.errors import (
    ...
    RPCError, UsernameInvalid, ChannelPrivate, ChatForwardsRestricted
)
...
# --- Web Series Regex (Optimized) ---
EPISODE_REGEX = re.compile(
    r"(.*?)(?:S|Season)\s*(\d{1,2})\s*(?:\s*E|Ep|Episode)\s*(\d{1,3})(?!.*\d)",
    re.IGNORECASE | re.DOTALL
)
EPISODE_PACK_REGEX = re.compile(
    r"(.*?)(?:S|Season)\s*(\d{1,2})\s*(?:\s*E|Ep|Episode)\s*(\d{1,3})\s*-\s*(\d{1,3})",
    re.IGNORECASE | re.DOTALL
)
    ...
    try:
        ...
        if ref_str.lstrip('-').isdigit():
            ...
            try:
                ...
                # Add check for CHANNEL_PRIVATE / USER_NOT_PARTICIPANT (useful for target check)
                if 'CHANNEL_PRIVATE' in str(e) or 'USER_NOT_PARTICIPANT' in str(e):
                    raise RuntimeError(f"❌ Resolve Failed: Bot user is not a member of {ref_str}. Join it first.")
...
* `/index_target_full <chat_id>` - Target ko scan k करके `full_target_index.json` banata hai.
...
def get_media_details(m):
    # FINAL OPTIMIZATION: Prioritize getting details from media objects known to carry files/videos.
    
    media = m.video
    if media:
        return media.file_name, media.file_size, media.file_unique_id

    # Check document/file. We must ensure it's not a common non-video file.
    media = m.document
    if media:
        if media.mime_type and not media.mime_type.startswith("video/") and not media.mime_type in ["application/x-bittorrent", "application/zip", "application/pdf"]:
             # Skip documents that are explicitly non-video and non-generic
             pass
        else:
             return media.file_name, media.file_size, media.file_unique_id
             
    # Fallback check for any media that carries File ID (aggressive attempt for Full Index)
    # This should help catch files wrapped in unusual media types, though it might grab photos/audio for full index.
    if m.media and hasattr(m.media, 'file_unique_id'):
        file_unique_id = getattr(m.media, 'file_unique_id', None)
        if file_unique_id:
             # Since we don't have file_name/size readily available for simple m.media, we might miss compound key match, but unique ID will be captured.
             # This is a compromise for aggressive indexing.
             return getattr(m.media, 'file_name', None), getattr(m.media, 'file_size', None), file_unique_id
...
async def index_channel_cmd(_, message):
    ...
    async def runner():
        ...
        try:
            ...
            # --- Indexing is Newest to Oldest (Default) ---
            status = await message.reply(f"⏳ Movie Indexing shuru ho raha hai: `{src_name}`...\n(यह बड़े channels को पूरी तरह से **Newest to Oldest** स्कैन करेगा)\n(Web series skip kar dega)")

            processed_count = 0
            found_count = 0
            
            try:
                # Messages को iterate करें (Newest to Oldest - Default)
                async for m in app.get_chat_history(src_id, limit=0, offset_id=0):
                    ...
                    # Media check (Optimized get_media_details used)
                    ...
                    # FIX: Status update frequency reduced for faster indexing (5000 messages)
                    if processed_count % 5000 == 0:
                        try: 
                            await status.edit(f"⏳ Indexing Movies...\nProcessed: {processed_count} messages\nFound: {found_count} unique movies")
                        except FloodWait: pass 
                
                if not GLOBAL_TASK_RUNNING: return

                save_movie_index_db()
                await status.edit(f"🎉 Movie Indexing Complete!\nChannel: `{src_name}`\nTotal Processed Messages: {processed_count}\nFound: **{found_count}** unique movies.\n\nDatabase ko `movie_database.json` me save kar diya hai.")

            except ChatForwardsRestricted:
                 await status.edit("❌ Indexing Failed: यह channel/group content forwarding को प्रतिबंधित करता है। आप messages को retrieve नहीं कर सकते।")
            except Exception as e:
...
async def index_target_cmd(_, message):
    ...
    async def runner():
        ...
        try:
            ...
            try:
                # Messages को iterate करें (Newest to Oldest - Default)
                async for m in app.get_chat_history(tgt_id, limit=0, offset_id=0):
                    if not GLOBAL_TASK_RUNNING:
                        await status.edit("🛑 Task stopped by user.")
                        break
                    ...
                    except Exception as e: print(f"[INDEX_TGT ERR] Msg {m.id}: {e}")
                    
                    # FIX: Status update frequency reduced for faster indexing (5000 messages)
                    if processed_count % 5000 == 0:
                        try: 
                            await status.edit(f"⏳ Indexing Target Movies...\nProcessed: {processed_count} messages\nFound: {len(target_unique_ids)} unique IDs")
                        except FloodWait: pass 
...
def parse_series_info(text):
    ...
    if match:
        name = match.group(1).strip()
        season = int(match.group(2))
        # FIX: complete season ke liye episode number 0 set karein
        return name, season, 0, None 

    # FIX: Episode Only Regex ko tabhi chalao jab title mein 'S' ya 'Season' keywords na hon
    if not SIMPLE_SEASON_REGEX.search(text):
            ...
            try:
                ...
                async for m in app.get_chat_history(src_id, limit=0, offset_id=0):
                    ...
                    try:
                        ...
                        # Fallback for simple Season tag without Episode
                        if not series_name:
                        ...
                        found_count += 1
                    except Exception as e: print(f"[INDEX_WS ERR] Msg {m.id}: {e}")
                    
                    # FIX: Status update frequency reduced for faster indexing (5000 messages)
                    if processed_count % 5000 == 0:
                        try: await status.edit(f"⏳ Indexing Web Series...\nProcessed: {processed_count} messages\nFound: {found_count} episodes")
                        except FloodWait: pass 
                ...
                # Sorting will still work correctly regardless of initial scan order
                ...
                await status.edit(f"🎉 Web Series Indexing & Sorting Complete!\nChannel: `{src_name}`\nTotal Processed Messages: {processed_count}\nFound: **{found_count}** total episodes.\n\nSorted database ko `webseries_database.json` me save kar diya hai.")
                 ...
                 await status.edit("❌ Indexing Failed: यह channel/group content forwarding को प्रतिबंधित करता है। आप messages को retrieve नहीं कर सकते हैं।")
            except Exception as e:
            ...
            try:
                ...
                async for m in app.get_chat_history(tgt_id, limit=0, offset_id=0):
                    ...
                    try:
                        ...
                        if file_name and file_size:
                            target_compound_keys_set.add(f"{file_name}-{file_size}")
                    except Exception as e: print(f"[INDEX_TGT_WS ERR] Msg {m.id}: {e}")

                    # FIX: Status update frequency reduced for faster indexing (5000 messages)
                    if processed_count % 5000 == 0:
                        try: await status.edit(f"⏳ Indexing Target Web Series...\nProcessed: {processed_count} messages\nFound: {len(target_unique_ids)} unique")
                        except FloodWait: pass
        ...
        try:
            ...
            try:
                ...
                async for m in app.get_chat_history(src_id, limit=0, offset_id=0):
                    ...
                    # Media check (Optimized get_media_details used - Should now catch all videos/docs/files)
                    file_name, file_size, unique_id = get_media_details(m)
                    if not unique_id: 
                        continue 
                    ...
                    # FIX: Status update frequency reduced for faster indexing (5000 messages)
                    if processed_count % 5000 == 0:
                        try: await status.edit(f"⏳ Indexing All Media...\nProcessed: {processed_count} messages\nFound: {found_count} media")
                        except FloodWait: pass 
                ...
                # FIX: Reverse list to get Oldest to Newest order for forwarding (since Pyrogram default is Newest to Oldest)
            ...
            try:
                ...
                async for m in app.get_chat_history(tgt_id, limit=0, offset_id=0):
                    ...
                    try:
                        ...
                        if file_name and file_size:
                            target_compound_keys_set.add(f"{file_name}-{file_size}")
                    except Exception as e: print(f"[INDEX_TGT_FULL ERR] Msg {m.id}: {e}")
                    
                    # FIX: Status update frequency reduced for faster indexing (5000 messages)
                    if processed_count % 5000 == 0:
                        try: await status.edit(f"⏳ Indexing Full Target...\nProcessed: {processed_count} messages\nFound: {len(target_unique_ids)} unique")
                        except FloodWait: pass
    ...
    async def runner():
        ...
        try:
            ...
            await status.edit_text(
                ...
                f"Target: `{tgt_name}`\n"
                f"Total Media in Index (Old to New): `{total_in_index}`\n"
...
async def clean_dupes_cmd(_, message):
    ...
    async def runner():
        ...
        try:
            ...
            seen_media = {} # file_name-file_size -> message_id
            messages_to_delete = []
            processed_count = 0

            try:
                # Messages को iterate करें (Newest to Oldest - Default)
                async for m in app.get_chat_history(chat_id, limit=0, offset_id=0):
                    if not GLOBAL_TASK_RUNNING:
                        await status.edit("🛑 Task stopped by user.")
                        break
                    ...
                    try:
                        ...
                        if compound_key in seen_media:
                            # Isko delete karo (jo baad mein mila hai)
                            messages_to_delete.append(m.id)
                        else:
                            # Isko rakho (jo pehle mila hai)
                            seen_media[compound_key] = m.id
                    except Exception as e: print(f"[CLEAN ERR] Msg {m.id}: {e}")
                ...
                if total_to_delete == 0:
                    await status.edit(f"🎉 Scan complete. Total Processed: {processed_count}. Koi 'Name+Size' duplicate nahi mila!")
                    return

                await status.edit(f"✅ Scan complete. Total Processed: {processed_count}.\nFound **{total_to_delete}** duplicates.\nAb 100 ke batch me delete karna shuru kar raha hoon...")
                ...
                await status.edit("❌ **Error: Main Admin nahi hoon!**\nMujhe channel/group mein 'Delete Messages' permission ke saath Admin banao.")
                 ...
                 await status.edit("❌ Scan Failed: यह channel/group content forwarding को प्रतिबंधित करता है। आप messages को retrieve नहीं कर सकते हैं।")
            except Exception as e:
        ...
        try:
            ...
            try:
                ...
                async for m in app.get_chat_history(chat_id, limit=0, offset_id=0):
                    ...
                    except Exception as e: print(f"[FIND BQ ERR] Msg {m.id}: {e}")
                ...
                if bad_quality_movies_list:
                    ...
                    await message.reply(f"🎉 Scan complete. Total Processed Messages: {processed_count}. Koi low-quality audio movie nahi mili (keywords ke hisaab se) `{chat_name}` mein.")
...
# --- /index_for_editing (Fixed for large channels) ---
                ...
                async for m in app.get_chat_history(src_id, limit=0, offset_id=0):
                    ...
                    try:
                        if m.caption:
                            ...
                            found_count += 1
                    except Exception as e: print(f"[INDEX_EDIT ERR] Msg {m.id}: {e}")
                    
                    # FIX: Status update frequency reduced for faster indexing (5000 messages)
                    if processed_count % 5000 == 0:
                        try: await status.edit(f"⏳ Indexing for Editing...\nProcessed: {processed_count} messages\nFound: {found_count} (To Edit)\nSkipped (Already OK): {skipped_due_to_lock}")
                        except FloodWait: pass 
                
                if not GLOBAL_TASK_RUNNING: return
                
                # Index is in Newest to Oldest order (Default)
        ...
        try:
            for i, batch in enumerate(batches):
                ...
                try:
                    ...
                    # Unhe एक dictionary me daalo taaki fast access ho sake
...
async def sync_chats(_, message):
    ...
    async def runner():
        ...
        try:
            ...
            # FIX: __anext__() is used for reliable async iteration
            while True:
                try:
                    dialog = await dialog_iterator.__anext__()
