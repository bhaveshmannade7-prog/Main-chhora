...
async def resolve_chat_id(client: Client, ref: str | int):
    ...
    if isinstance(ref, int) or (isinstance(ref, str) and ref.lstrip("-").isdigit()):
        try:
            ...
            # अगर Numeric ID fail होती है, तो यह Peer ID known नहीं है।
            raise RuntimeError("❌ Peer ID not known. **सुनिश्चित करें कि User Account इस चैट में है या /sync चलाएं।**")
        except RPCError as e:
            # अन्य RPC errors को यहाँ पकड़ें (जैसे ACCESS_DENIED)
            raise RuntimeError(f"❌ Resolve Failed (Numeric ID): {e}")
        except Exception:
            # अगर int() conversion या अन्य error आता है, तो आगे बढ़ें
            pass
