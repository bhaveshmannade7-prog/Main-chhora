...
async def index_channel_cmd(_, message):
    ...
    async def runner():
        ...
        try:
            ...
            # --- FIX: Removed reverse=True. Now scans Newest to Oldest. ---
            status = await message.reply(f"⏳ Movie Indexing shuru ho raha hai: `{src_name}`...\n(यह बड़े channels को पूरी तरह से **Newest to Oldest** scan करेगा)\n(Web series skip kar dega)")
            ...
            try:
                ...
                async for m in app.get_chat_history(src_id, limit=0, offset_id=0):
                    ...
                    # FIX: Status update frequency reduced for faster indexing
        ...
        try:
            ...
            try:
                ...
                async for m in app.get_chat_history(tgt_id, limit=0, offset_id=0):
                    ...
                    try:
                        text_to_check = (getattr(media, 'file_name', "") or "") + " " + (m.caption or "")
                        # Web Series check
                        if SERIES_KEYWORDS_REGEX.search(text_to_check):
                            continue
...
async def index_full_cmd(_, message):
    ...
    async def runner():
        ...
        try:
            ...
            # --- FIX: Removed reverse=True. Now scans Newest to Oldest, then reverses the list at the end. ---
