        ...
        try:
            ...
            try:
                ...
                async for m in app.search_messages(src_id, filter=enums.MessagesFilter.DOCUMENT, limit=0):
                    ...
                    processed_stage2 += 1
                    try:
                        # FIX: MIME and extension check optimized to catch all movies
                        if not (m.document and (m.document.mime_type.startswith("video/") or (m.document.file_name and m.document.file_name.lower().endswith(('.mkv', '.mp4', '.avi', '.wmv', '.mov', '.ts', '.m4v', '.webm'))))): continue
        ...
        try:
            ...
            try:
                ...
                async for m in app.search_messages(src_id, filter=enums.MessagesFilter.DOCUMENT, limit=0):
                    ...
                    processed_stage2 += 1
                    try:
                        # Broad indexing to catch everything, but keep it relevant for forward
                        file_name, file_size, unique_id = get_media_details(m)
                        if not unique_id: continue
        ...
        try:
            ...
            while True:
                try:
                    ...
                    # यह सुनिश्चित करने के लिए कि dialog object ठीक से lode ho
                    _ = dialog.chat.id 
                    count += 1
                    ...
                    # RPCError (जैसे CHANNEL_PRIVATE, USERNAME_INVALID) ko skip karein
            ...
            # Outer block if iterator itself crashes
